...
const BlogContentRaw = ({ RAW }: { RAW: any[] }) => {
  return (
    <>
      {RAW.map((v, i: number) => {
        if (v.type == 'paragraph') {
          ...
          return (
            <h1 key={i} className='pb-4 pt-24 text-5xl font-semibold'>
              <ObjectText content={v.children} />
            </h1>
          )
        } else if (v.type == 'heading-two') {
          return (
            <h2 key={i} className='pb-4 pt-24 text-4xl font-semibold'>
              <ObjectText content={v.children} />
            </h2>
          )
        } else if (v.type == 'heading-three') {
          return (
            <h3 key={i} className='pb-4 pt-24 text-3xl font-semibold'>
              <ObjectText content={v.children} />
            </h3>
          )
          ...
          return (
            <blockquote
              key={i}
              className='my-12 rounded-md border-4 border-y-white/0 border-l-primary-0
            border-r-white/0 bg-black/40 p-6 text-3xl font-light leading-10'
            >
              <ObjectText content={v.children} />
            </blockquote>
          )
...
const ObjectText = ({ content }: { content: any[] }) => {
  return (
    <>
...
export { BlogContentRaw, ObjectText }
